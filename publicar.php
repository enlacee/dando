...
$categorias = $instancia->getCategorias();
	...
	if(array_key_exists("publicar",$_POST)){
		$publicacion = $instancia->setProducto(Apps::acentos($_POST["categoriaID"]),$_SESSION["email"],$_SESSION["email"],"1",Apps::acentos($_POST["titulo"]),Apps::acentos($_POST["foto"]),Apps::acentos($_POST["video"]),"disponible",$_POST["precio"],Apps::acentos($_POST["pais"]),Apps::acentos($_POST["ciudad"]),Apps::acentos($_POST["detalles"]),$_SERVER["REMOTE_ADDR"],date("d/m/Y"),date("h:m:i"));
	}
